        ...
        >
          ...
          >
            <DeleteConfirmation
              ...
              itemName={`page "${page.title || "Untitled"}"`}
              position="bottom"
            />
          </Box>
        </Box>
...
const Sidebar: React.FC<SidebarProps> = observer(({ isOpen }) => {
  ...
  const handlePageSelectFromSearch = async (pageId: string) => {
    const searchResult = searchStore.results.find((r) => r.id === pageId);
    if (!searchResult) return;

    try {
      // Use the page store's selectPageFromUrl method which handles:
      // 1. Checking if page is already in memory
      // 2. Fetching from backend if not found
      // 3. Properly loading with correct content format
      await pageStore.selectPageFromUrl(pageId);

      // Keep search state but hide search input after a short delay to allow navigation
      setTimeout(() => {
        setSearchMode("simple");
        setShowSearchInput(false);
        searchStore.clearSearch();
      ...
      console.error("Error selecting page from search:", error);
      // Fallback: clear search and show error if needed
      setTimeout(() => {
        setSearchMode("simple");
        setShowSearchInput(false);
        searchStore.clearSearch();
  ...
  const handleRestorePage = (pageId: string) => {
    pageStore.restorePage(pageId);
  };

  const handlePermanentlyDeletePage = (pageId: string) => {
    pageStore.permanentlyDeletePage(pageId);
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Conditional: Back to Pages OR New Page */}
          <Box mb="sm">
            {uiStore.sidebarView === "trash" ? (
              // Back Button - only show in trash view
              <Box
                onClick={() => uiStore.setSidebarView("pages")}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: "8px",
              ...
              >
                <FontAwesomeIcon icon={faArrowLeft} size="sm" color="#666" />
                <Text size="sm" color="#666">
                  Back to Pages
                </Text>
              </Box>
            ) : (
              // New Page - only show in pages view
              <Box
